 ...
 *  调用上列自定义方法 setNumberPicker() 初始化数据；
 */
...
//            throw new IllegalArgumentException("should not set a value greater than mMaxValue, value is " + value);
    ...
    public String getContentByCurrValue(){
        int index = getValue() - mMinValue;
        if( index >= mDisplayedValues.length ){
            return "";
        }
        return mDisplayedValues[index];
    }
...
//                    + " getOneRecycleSize() : " + getOneRecycleSize() + " mWrapSelectorWheel : " + mWrapSelectorWheel);
            if( index < 0 ){
                ...
                return getOneRecycleSize() + mMinShowIndex;
            }
        }
    }
